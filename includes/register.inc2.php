...
/* // Insert the new user into the database 
        if ($insert_stmt = $mysqli->prepare("INSERT INTO members (email, username, fname, lname, sClass) VALUES (?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('sssss', $username, $fname, $lname, $hname, $hcity, $hstate, $hcode, $sClass, $email, $ccard, $password, $random_salt);
            // Execute the prepared query.*/
 
 
        // Insert the new user into the database Members Table
        if ($insert_stmt = $mysqli->prepare("INSERT INTO members (email, username, fname, lname, sClass) VALUES (?, ?, ?, ?, ?)")) {
			...
			if ($insert_stmt = $mysqli->prepare("INSERT INTO members_addresses (email, hname, hcity, hstate, hcode) VALUES (?, ?, ?, ?, ?)")) {
			...
			if ($insert_stmt = $mysqli->prepare("INSERT INTO members_password (email, password, salt) VALUES (?, ?, ?,)")) {
